...
const VanDetails = () => {
  ...
  return (
    <div style={vans ? {} : { height: "100vh"}} className="flex items-start flex-col px-5 my-5 gap-4">
        <Link className=" flex items-center gap-2 justify-center w-[142px] h-[23px]" to={`/vans`}>
            <img className="h-[10px]"  src="/Arrow 1.png" alt="" />
            <h3 className=" inline text-[16px] font-medium hover:underline"> Back to all vans </h3>
        </Link>
      <img
      ...
      <h2 className="text-black text-[20px] font-bold">${van?.price}<h3 className="text-[14px] inline font-semibold">/day</h3></h2>
     
      <p className="text-[16px] font-medium">{van?.description}</p>
      <button className=" w-[495px] h-[50px] mb-20 text-white flex justify-center items-center rounded-[5px] bg-[#FF8C38]" >Rent this van</button>
    </div>
  );
};
